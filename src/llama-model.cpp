...
struct llama_model::impl {
    ...
    std::unordered_map<ggml_backend_buffer_t, std::unique_ptr<llama_mlock>> mlock_bufs;
    ...
    // lazy weight loading state
    std::mutex lazy_load_mutex;
    ...
    std::vector<std::unique_ptr<lazy_weight_group>> lazy_layer_groups;
    lazy_weight_group lazy_output_group;
    std::unordered_map<ggml_tensor *, lazy_weight_group *> lazy_tensor_to_group;
    bool lazy_alloc_done = false;
    bool lazy_load_done   = false;
    bool lazy_load_failed = false;
    bool lazy_load_async_started = false;
...
static bool llama_tensor_has_backing_memory(const ggml_tensor * t) {
    return t->buffer != nullptr && t->data != nullptr && ggml_backend_buffer_get_size(t->buffer) > 0;
}

static bool llama_tensor_depends_on(const ggml_tensor * t, const ggml_tensor * root) {
...
struct llama_tensor_location {
    ggml_context * ctx = nullptr;
    std::vector<ggml_backend_buffer_ptr> * bufs = nullptr;
};

static std::optional<llama_tensor_location> llama_find_tensor_location(
        std::vector<std::pair<ggml_context_ptr, std::vector<ggml_backend_buffer_ptr>>> & ctxs_bufs,
        ggml_tensor * target) {
    for (auto & [ctx_ptr, bufs] : ctxs_bufs) {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx_ptr.get()); t != nullptr; t = ggml_get_next_tensor(ctx_ptr.get(), t)) {
            if (t == target) {
                return llama_tensor_location{ctx_ptr.get(), &bufs};
            }
        }
    }
    return std::nullopt;
...
static int llama_parse_layer_from_tensor_name(const char * name) {
...
static bool llama_tensor_name_is_output(const char * name) {
...
static ggml_backend_dev_t llama_get_buft_device_or_cpu(ggml_backend_buffer_type_t buft, const char * func) {
    ggml_backend_dev_t dev = ggml_backend_buft_get_device(buft);
    if (!dev) {
        dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
        if (!dev) {
            throw std::runtime_error(format("%s: no CPU backend found", func));
        }
    }
    return dev;
}
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    pimpl->lazy_global_group.tensors.clear();
    pimpl->lazy_output_group.tensors.clear();
    pimpl->lazy_layer_groups.clear();
    pimpl->lazy_layer_groups.reserve(hparams.n_layer);
    pimpl->lazy_tensor_to_group.clear();
    for (uint32_t il = 0; il < hparams.n_layer; ++il) {
        pimpl->lazy_layer_groups.emplace_back(new impl::lazy_weight_group);
    }
    ...
    for (auto & [name, tensor] : tensors_by_name) {
        ...
        impl::lazy_weight_group * group = nullptr;
        const int il = llama_parse_layer_from_tensor_name(name.c_str());
        if (il >= 0 && il < (int) pimpl->lazy_layer_groups.size()) {
            group = pimpl->lazy_layer_groups[il].get();
        } else if (llama_tensor_name_is_output(name.c_str())) {
            group = &pimpl->lazy_output_group;
        } else {
            group = &pimpl->lazy_global_group;
        }

        group->tensors.push_back(tensor);
        pimpl->lazy_tensor_to_group.emplace(tensor, group);
    }

    const bool lazy_load = !ml.no_alloc;

    // create placeholder backend buffers: tensor metadata is initialized here, concrete
    // allocation + weight loading is deferred to the first inference call.
    std::vector<std::pair<ggml_context *, llama_buf_map>> ctx_buf_maps;
    ctx_buf_maps.reserve(ml.ctx_map.size());

    // Ensure we have enough capacity for all backend buffers we will potentially create.
    // In strict per-tensor mode this is close to the number of tensors.
    const size_t n_max_backend_buffer = std::max<size_t>(ml.n_tensors, ml.ctx_map.size() * std::max<size_t>(1, ml.files.size()));
    pimpl->ctxs_bufs.reserve(n_max_backend_buffer);
    pimpl->lazy_ctx_buft.reserve(ml.ctx_map.size());
    ...
    for (auto & [buft, ctx_ptr] : ml.ctx_map) {
        ...
        if (buf == nullptr) {
            throw std::runtime_error(format("unable to allocate %s placeholder buffer", ggml_backend_buft_name(buft)));
        }
        ...
        pimpl->lazy_ctx_buft.emplace_back(ctx, buft);
    ...
    std::map<std::string, size_t> model_buf_size_by_name;
    for (auto & [_, bufs] : pimpl->ctxs_bufs) {
        for (auto & buf: bufs) {
            model_buf_size_by_name[ggml_backend_buffer_name(buf.get())] += ggml_backend_buffer_get_size(buf.get());
        }
    }
    for (const auto & [name, size] : model_buf_size_by_name) {
    ...
    if (lazy_load) {
        pimpl->lazy_ctx_buf_maps = std::move(ctx_buf_maps);
        pimpl->lazy_loader.reset(new llama_model_loader(std::move(ml)));
        pimpl->lazy_alloc_done = false;
        pimpl->lazy_load_done   = false;
        pimpl->lazy_load_failed = false;
        pimpl->lazy_load_async_started = false;
        pimpl->lazy_load_async_in_progress = false;
        pimpl->lazy_load_error.clear();
        ...
        reset_group(pimpl->lazy_output_group);
        LLAMA_LOG_INFO("%s: deferring weight allocation/load%s\n", __func__,
            params.parallel_load ? " until context init background preload" : " until first inference");
        return true;
    }
...
void llama_model::start_async_tensors_load() {
    if (!params.parallel_load || hparams.vocab_only || hparams.no_alloc) {
        return;
    }

    bool start = false;
    {
        std::lock_guard<std::mutex> lock(pimpl->lazy_load_mutex);
        if (!pimpl->lazy_loader || pimpl->lazy_load_done || pimpl->lazy_load_failed || pimpl->lazy_load_async_started) {
            return;
        }
        pimpl->lazy_load_async_started = true;
        pimpl->lazy_load_async_in_progress = true;
        start = true;
    }
    ...
    pimpl->lazy_load_thread = std::thread([this] {
        std::string err_msg;
        if (!this->ensure_tensors_ready(err_msg)) {
            LLAMA_LOG_ERROR("%s: async tensor loading failed: %s\n", __func__, err_msg.c_str());
        }
    });

    LLAMA_LOG_INFO("%s: started background tensor loading\n", __func__);
}

void llama_model::wait_async_tensors_load() {
    if (pimpl->lazy_load_thread.joinable()) {
        if (pimpl->lazy_load_thread.get_id() == std::this_thread::get_id()) {
            return;
        }
...
static void llama_refresh_tensor_views(ggml_context * ctx) {
    ...
    do {
        changed = false;
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            if (t->view_src != nullptr && t->buffer == nullptr && t->view_src->buffer != nullptr && t->view_src->data != nullptr) {
                if (ggml_backend_view_init(t) != GGML_STATUS_SUCCESS) {
                    throw std::runtime_error(format("failed to initialize tensor view %s", ggml_get_name(t)));
                }
                t->weight_buffer = nullptr;
                changed = true;
            }
...
static void llama_refresh_weight_handles(ggml_context * ctx) {
    for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
        t->weight_buffer = (t->view_src == nullptr &&
                            t->data != nullptr &&
                            t->buffer != nullptr &&
...
static void llama_ensure_tensor_allocated(
    ...
    const size_t max_tensor_size = ggml_get_max_tensor_size(ctx);
    ggml_backend_dev_t dev = llama_get_buft_device_or_cpu(buft, __func__);
    ...
    const bool try_tensor_mmap = ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft;

    ggml_backend_buffer_t buf = nullptr;

    if (try_tensor_mmap && weight != nullptr && alloc_size == ggml_nbytes(t)) {
        const auto & mapping = ml.mappings.at(weight->idx);
        uint8_t * data = (uint8_t *) mapping->addr() + weight->offs;
        buf = ggml_backend_dev_buffer_from_host_ptr(dev, data, alloc_size, max_tensor_size);
        if (buf == nullptr) {
            throw std::runtime_error(format("unable to allocate mmap-backed %s tensor buffer for %s",
                ggml_backend_buft_name(buft), ggml_get_name(t)));
        }
        ggml_backend_buffer_set_usage(buf, GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
        const enum ggml_status status = ggml_backend_tensor_alloc(buf, t, data);
        if (status != GGML_STATUS_SUCCESS) {
            ggml_backend_buffer_free(buf);
            throw std::runtime_error(format("failed to initialize mmap-backed tensor %s", ggml_get_name(t)));
        }

        if (use_mlock) {
            const auto & lmlock = mlock_mmaps.at(weight->idx);
            lmlock->grow_to(weight->offs + ggml_nbytes(t));
        }
        auto & mmap_used = ml.mmaps_used[weight->idx];
        mmap_used.first  = std::min(mmap_used.first,  weight->offs);
        mmap_used.second = std::max(mmap_used.second, weight->offs + ggml_nbytes(t));
    } else {
        buf = ggml_backend_buft_alloc_buffer(buft, alloc_size);
        if (buf == nullptr) {
            throw std::runtime_error(format("unable to allocate %s tensor buffer for %s",
                ggml_backend_buft_name(buft), ggml_get_name(t)));
        }
        ggml_backend_buffer_set_usage(buf, GGML_BACKEND_BUFFER_USAGE_WEIGHTS);

        void * base = ggml_backend_buffer_get_base(buf);
        const enum ggml_status status = ggml_backend_tensor_alloc(buf, t, base);
        if (status != GGML_STATUS_SUCCESS) {
            ggml_backend_buffer_free(buf);
            throw std::runtime_error(format("failed to initialize tensor %s", ggml_get_name(t)));
        }

        if (use_mlock && ggml_backend_buffer_is_host(buf)) {
            std::unique_ptr<llama_mlock> mlock_buf(new llama_mlock);
            mlock_buf->init   (ggml_backend_buffer_get_base(buf));
            mlock_buf->grow_to(ggml_backend_buffer_get_size(buf));
            mlock_bufs.emplace(buf, std::move(mlock_buf));
        }
    }

    if (weight != nullptr && buf_map != nullptr) {
        buf_map->try_emplace(weight->idx, buf);
    }

    bufs.emplace_back(buf);
}

static void llama_load_tensor_data(
        llama_model_loader & ml,
        bool use_mlock,
        llama_mlocks & mlock_mmaps,
        ggml_tensor * t) {
    ...
    if (ml.files.empty()) {
        ml.set_tensor_data(t, ml.set_tensor_data_ud);
        return;
    }

    const size_t n_size = ggml_nbytes(t);
    ...
    if (ml.use_mmap) {
        ...
        ggml_backend_tensor_set(t, data, 0, n_size);
    ...
    auto & file = ml.files.at(weight->idx);
...
struct llama_model_lazy_access {
static void prepare_lazy_allocations(llama_model & model, llama_load_timing_stats * timing_stats) {
    llama_model_loader & ml = *model.pimpl->lazy_loader;
    const bool use_mlock = model.params.use_mlock;

    ml.init_mappings(true, use_mlock ? &model.pimpl->mlock_mmaps : nullptr);
    model.pimpl->mappings.clear();
    model.pimpl->mappings.reserve(ml.mappings.size());
    model.pimpl->mlock_bufs.clear();
    ...
    std::unordered_map<ggml_context *, llama_buf_map *> ctx_buf_map_by_ptr;
    ctx_buf_map_by_ptr.reserve(model.pimpl->lazy_ctx_buf_maps.size());
    for (auto & [ctx, buf_map] : model.pimpl->lazy_ctx_buf_maps) {
        ctx_buf_map_by_ptr.emplace(ctx, &buf_map);
    }
    ...
    for (auto & [ctx_ptr, bufs] : model.pimpl->ctxs_bufs) {
        ...
        auto it_buft = ctx_buft_by_ptr.find(ctx);
        if (it_buft == ctx_buft_by_ptr.end()) {
            throw std::runtime_error("internal error: missing buffer type for model context");
        }
        ggml_backend_buffer_type_t buft = it_buft->second;

        auto it_buf_map = ctx_buf_map_by_ptr.find(ctx);
        if (it_buf_map == ctx_buf_map_by_ptr.end()) {
            throw std::runtime_error("internal error: missing source buffer map for model context");
        }
        llama_buf_map & buf_map = *it_buf_map->second;
        buf_map.clear();

        // release placeholder buffers, then reset tensor pointers for final allocation.
        bufs.clear();
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            t->buffer        = nullptr;
            t->weight_buffer = nullptr;
        ...
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            if (t->view_src != nullptr || t->buffer != nullptr || t->data != nullptr) {
                continue;
            }
    ...
    model.pimpl->lazy_alloc_done = true;
}

static void load_tensor_group_data(
        llama_model & model,
    ...
    for (ggml_tensor * t : group.tensors) {
        ...
        if (model.params.progress_callback) {
            const float progress = ml.size_data == 0 ? 0.0f : (float) ml.size_done / ml.size_data;
            if (!model.params.progress_callback(progress, model.params.progress_callback_user_data)) {
    ...
    for (ggml_tensor * t : group.tensors) {
        ...
        if (t->data == nullptr || t->buffer == nullptr) {
            throw std::runtime_error(format("tensor %s has no backing memory before group load", ggml_get_name(t)));
        }
        tasks.push_back({ t, weight->idx, weight->offs, ggml_nbytes(t) });
    }

    const bool can_async_io = model.params.async_io_load && !model.params.use_mlock && !ml.files.empty() && !ml.use_mmap && !tasks.empty();
    if (can_async_io) {
        const size_t n_queues = llama_async_io_queue_count(tasks.size());
        size_t max_non_host_tensor_size = 0;
        for (const auto & task : tasks) {
        ...
        auto queues = llama_build_contiguous_io_queues(std::move(tasks), n_queues);
        if (!queues.empty()) {
            LLAMA_LOG_INFO("llama_load_all_tensor_data_async_io: loading tensor data with %zu async IO queues\n", queues.size());
            ...
            if (staging_bytes > 0) {
                LLAMA_LOG_INFO("llama_load_all_tensor_data_async_io: preallocated %.2f MiB staging buffers for async IO\n",
    ...
    if (model.params.progress_callback) {
        const float progress = ml.size_data == 0 ? 1.0f : std::min(1.0f, (float) ml.size_done / ml.size_data);
        if (!model.params.progress_callback(progress, model.params.progress_callback_user_data)) {
            throw std::runtime_error("weight loading cancelled by progress callback");
        }
    }
...
static void set_all_group_states(
        llama_model & model,
        llama_model::impl::lazy_group_state state,
        const std::string & error = {}) {
    set_group_state(model.pimpl->lazy_global_group, state, error);
    for (auto & group : model.pimpl->lazy_layer_groups) {
        set_group_state(*group, state, error);
    }
    set_group_state(model.pimpl->lazy_output_group, state, error);
}

static void load_all_tensor_data(
        llama_model & model,
        llama_load_timing_stats * timing_stats) {
    if (!llama_load_all_tensor_data_per_tensor(
                *model.pimpl->lazy_loader,
                model.params.use_mlock,
                model.pimpl->mlock_mmaps,
                model.pimpl->ctxs_bufs,
                model.params.progress_callback,
                model.params.progress_callback_user_data,
                timing_stats,
                model.params.async_io_load)) {
        throw std::runtime_error("weight loading cancelled by progress callback");
    }
}

static bool ensure_group_ready(
        llama_model & model,
        llama_model::impl::lazy_weight_group & group,
        std::string & err_msg,
        llama_load_timing_stats * timing_stats = nullptr) {
    err_msg.clear();
...

    {
        ...
        if (group.state == llama_model::impl::lazy_group_state::failed) {
            err_msg = group.error;
            return false;
        }
...
bool llama_model::ensure_global_tensors_ready(std::string & err_msg) {
    return llama_model_lazy_access::ensure_group_ready(*this, pimpl->lazy_global_group, err_msg);
}

bool llama_model::ensure_layer_tensors_ready(int il, std::string & err_msg) {
    if (il < 0 || il >= (int) pimpl->lazy_layer_groups.size()) {
        err_msg = format("layer index %d is out of range", il);
        return false;
    }
    return llama_model_lazy_access::ensure_group_ready(*this, *pimpl->lazy_layer_groups[il], err_msg);
}
...
bool llama_model::ensure_tensors_ready(std::string & err_msg) {
    ...
    if (pimpl->lazy_load_done) {
        return true;
    }

    if (pimpl->lazy_load_failed) {
        err_msg = pimpl->lazy_load_error;
        return false;
    }

    if (!pimpl->lazy_loader) {
    ...
    try {
...

        {
            ...
            if (pimpl->lazy_load_failed) {
                throw std::runtime_error(pimpl->lazy_load_error);
            }
        ...
        for (const auto & [ctx_ptr, _] : pimpl->ctxs_bufs) {
            for (ggml_tensor * t = ggml_get_first_tensor(ctx_ptr.get()); t != nullptr; t = ggml_get_next_tensor(ctx_ptr.get(), t)) {
                const bool has_real_memory = t->buffer != nullptr && ggml_backend_buffer_get_size(t->buffer) > 0 && t->data != nullptr;
                if (!has_real_memory) {
        ...
        pimpl->lazy_load_done = true;
        pimpl->lazy_load_async_in_progress = false;
        llama_model_lazy_access::set_all_group_states(*this, impl::lazy_group_state::ready);
        return true;
    } catch (const std::exception & e) {
        pimpl->lazy_load_failed = true;
        pimpl->lazy_load_error = e.what();
        pimpl->lazy_load_async_in_progress = false;
        llama_model_lazy_access::set_all_group_states(*this, impl::lazy_group_state::failed, pimpl->lazy_load_error);
        err_msg = pimpl->lazy_load_error;
        return false;
    }
}
...
static bool buft_supported(ggml_backend_buffer_type_t buft, ggml_backend_dev_t dev, F & fn) {
    ...
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        if (op_tensor->src[i] != nullptr) {
            ...
            op_tensor->src[i]->weight_buffer = nullptr;
        }
    }
...
bool llama_model::is_tensor_loaded(const char * name) const {
    const ggml_tensor * tensor = get_tensor(name);
    return tensor != nullptr && tensor->view_src == nullptr && llama_tensor_has_backing_memory(tensor);
}
...
bool llama_model::unload_tensor(const char * name, std::string & err_msg, size_t * bytes_freed) {
    ...
    if (hparams.vocab_only || hparams.no_alloc) {
        err_msg = "model weights are not allocated";
        return false;
    }

    ggml_tensor * tensor = const_cast<ggml_tensor *>(get_tensor(name));
    if (tensor == nullptr) {
        err_msg = format("tensor '%s' not found", name);
        return false;
    }

    if (tensor->view_src != nullptr) {
        err_msg = format("tensor '%s' is a view and cannot be unloaded independently", name);
        return false;
    }

    std::lock_guard<std::mutex> lock(pimpl->lazy_load_mutex);

    if (!llama_tensor_has_backing_memory(tensor)) {
        err_msg = format("tensor '%s' is already unloaded", name);
        return false;
    }

    auto loc = llama_find_tensor_location(pimpl->ctxs_bufs, tensor);
    if (!loc.has_value()) {
        err_msg = format("internal error: tensor '%s' was not found in model contexts", name);
        return false;
    }

    ggml_backend_buffer_t buf = tensor->buffer;
    const size_t freed = ggml_backend_buffer_get_size(buf);

    for (ggml_tensor * t = ggml_get_first_tensor(loc->ctx); t != nullptr; t = ggml_get_next_tensor(loc->ctx, t)) {
        if (!llama_tensor_depends_on(t, tensor)) {
            continue;
        }
        t->buffer = nullptr;
        t->weight_buffer = nullptr;
    ...
    pimpl->mlock_bufs.erase(buf);

    auto & bufs = *loc->bufs;
    bufs.erase(std::remove_if(bufs.begin(), bufs.end(),
        [buf](const ggml_backend_buffer_ptr & cur) {
            return cur.get() == buf;
        }), bufs.end());
    ...
    auto it_group = pimpl->lazy_tensor_to_group.find(tensor);
    if (it_group != pimpl->lazy_tensor_to_group.end()) {
